...
# npx ts-node src/setStipend.ts |tee -a logs/log-${DATE}-rewards-no-stipend.txt
npx ts-node src/send-in-one-batch.ts |tee -a logs/log-${DATE}-rewards-no-stipend.txt
npx ts-node src/check.ts 2>&1 |tee -a logs/log-${DATE}-check-no-stipend.txt
